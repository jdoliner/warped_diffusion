...
class ThinPlateSplineWarp(nn.Module):
    ...
    def _solve_tps_coefficients(
        self, src_points: torch.Tensor, dst_points: torch.Tensor
    ...
    def compute_warp_grid(
        self, src_points: torch.Tensor, dst_points: torch.Tensor, output_size: Tuple[int, int]
    ) -> torch.Tensor:
        ...
        W_tps, A = self._solve_tps_coefficients(src_points, dst_points)

        # Create a regular grid of query points
        y = torch.linspace(-1, 1, out_H, device=device, dtype=dtype)
        x = torch.linspace(-1, 1, out_W, device=device, dtype=dtype)
        ...
        # Apply TPS transformation: f(p) = A @ [1, p_x, p_y]^T + K @ W_tps
        n_query = out_H * out_W
        ...
        # TPS part: [B, H*W, 2]
        tps = torch.bmm(K, W_tps)

        # Combined transformation
        warped = affine + tps  # [B, H*W, 2]

        # Reshape to grid format
        warp_grid = warped.reshape(batch_size, out_H, out_W, 2)
    ...
    def warp(
        ...
        warped = F.grid_sample(
            tensor, grid, mode=mode, padding_mode=padding_mode, align_corners=True
        )
